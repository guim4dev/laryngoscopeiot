<template>
  <div class="sensors-wrapper">
    <VerticalBarSensor
      :value="tongueForceSensorDefinition.value.value"
      :max-value="maxForceSensorNormalizedValue"
      :danger-threshold="dangerThreshold"
      :warning-threshold="warningThreashold"
      :label="tongueForceSensorDefinition.label"
      icon-path="/icons/pressure_colorful.png"
      class="vertical-sensor"
    />
    <MeasurementHistoryChart
      :measurements="measurementHistory"
      :max-y="maxForceSensorNormalizedValue"
      :total-points="measuresHistoryMaxLength"
      class="measurements-history"
    />
  </div>
</template>
...
const maxForceSensorNormalizedValue = 50; // 50N
const warningThreashold = 30;
const dangerThreshold = 40;
const props = defineProps<{ src: string }>();
const emit = defineEmits(["started", "lostConnection"]);

// values range from 0 to 4095
// we need to normalize it to a range from 0N to 50N of force.
...
// measurements arrive every 300ms. We desire to store the measurements for the last 30 seconds
const measuresHistoryMaxLength = 100;
...
const handleTeethBeingTouched = () => {
  if (!teethMessageHandler) {
    teethMessageHandler = ElMessage({
      message: t("sensors.teeth_being_touched"),
      type: "error",
      duration: 0,
      offset: messagePositionOffset,
    });
  }
...
watch(data, (newData) => {
  ...
  addMeasurementToHistory({
    teethPressed,
    tongueForce: tongueForceSensor.value,
    timestamp: lastReceivedMessageAt.getTime(),
  });
});
...
onMounted(() => {
  setInterval(() => {
    if (lastReceivedMessageAt && isConnected.value) {
      ...
      if (diff > 5000) {
        // 5 seconds without receiving a message
        handleLostConnection();
      }
    }
